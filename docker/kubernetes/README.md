...
- The main backend deployment must provide the shared Kafka broker through a
  Service named `bank-kafka` on port `9092`.
...
```text
exchange-rate-service.bank.svc.cluster.local:50053
```

Redis is available only inside the cluster at:

```text
exchange-rate-redis.bank.svc.cluster.local:6379
```
...
```text
bank-kafka.bank.svc.cluster.local:9092
```
